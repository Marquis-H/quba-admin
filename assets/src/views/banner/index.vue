<template>
  <div>
    <h4 class="font-weight-bold py-2 mb-2">Banner管理</h4>
    <list-filter
...
export default {
  ...
  methods: {
    ...
    // 保存标签
    handleCreateData () {
      createBanner(this.userForm).then((result) => {
    ...
    handleUpdateData () {
      ...
      updateBanner(id, this.userForm).then((result) => {
    ...
    // 删除标签
    handleDel (row) {
      deleteBanner(row.id, row).then((result) => {
