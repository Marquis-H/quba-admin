<template>
  <div>
    <h4 class="font-weight-bold py-2 mb-2">闲置分类管理</h4>
    <list-filter
...
import { getIdleCategoryList, createIdleCategory, updateIdleCategory, deleteIdleCategory } from '@/api/idleCategory'
...
export default {
  ...
  methods: {
    getList () {
      getIdleCategoryList(this.listQuery)
        .then(res => {
    ...
    // 保存标签
    handleCreateData () {
      createIdleCategory(this.userForm).then((result) => {
    ...
    handleUpdateData () {
      ...
      updateIdleCategory(id, this.userForm).then((result) => {
    ...
    // 删除标签
    handleDel (row) {
      deleteIdleCategory(row.id, row).then((result) => {
