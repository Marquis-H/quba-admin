<template>
  ...
  >
    <template slot="isOnline" slot-scope="data">
      <b-badge variant="outline-success" v-if="data.item.isOnline === true"
        >线上</b-badge
      >
      <b-badge variant="outline-danger" v-if="data.item.isOnline === false"
        >线下</b-badge
      >
    </template>
    <template slot="type" slot-scope="data">
      <span>{{ data.item.type | getText("types", options) }}</span>
    </template>
...
export default {
  ...
  filters: {
    getText: function (value, type, options) {
      return options.types.find(v => v.value === value).text
    }
  },
  methods: {
