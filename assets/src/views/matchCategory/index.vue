<template>
  <div>
    <h4 class="font-weight-bold py-2 mb-2">比赛类别管理</h4>
    <list-filter
...
import { getMatchCategoryList, createMatchCategory, updateMatchCategory, deleteMatchCategory } from '@/api/matchCategory'
...
export default {
  ...
  methods: {
    getList () {
      getMatchCategoryList(this.listQuery)
        .then(res => {
    ...
    // 保存标签
    handleCreateData () {
      createMatchCategory(this.userForm).then((result) => {
    ...
    handleUpdateData () {
      ...
      updateMatchCategory(id, this.userForm).then((result) => {
    ...
    // 删除标签
    handleDel (row) {
      deleteMatchCategory(row.id, row).then((result) => {
