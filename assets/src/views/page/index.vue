<template>
  <div>
    <h4 class="font-weight-bold py-2 mb-2">页面管理</h4>
    <list-filter
...
export default {
  ...
  methods: {
    ...
    // 保存标签
    handleCreateData () {
      createPage(this.userForm).then((result) => {
    ...
    handleUpdateData () {
      ...
      updatePage(id, this.userForm).then((result) => {
    ...
    // 删除标签
    handleDel (row) {
      deletePage(row.id, row).then((result) => {
