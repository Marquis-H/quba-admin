<template>
  <div>
    <h4 class="font-weight-bold py-2 mb-2">专业管理</h4>
    <list-filter
...
import { getProfessionalList, createProfessional, updateProfessional, deleteProfessional } from '@/api/professional'
import { getCollegeItems } from '@/api/college'
...
export default {
  ...
  methods: {
    getList () {
      ...
      getCollegeItems().then(res => {
        this.options.colleges = res.data
      })
    },
    handleFilter () {
      this.getList()
    ...
    // 保存标签
    handleCreateData () {
      createProfessional(this.userForm).then((result) => {
    ...
    handleUpdateData () {
      ...
      updateProfessional(id, this.userForm).then((result) => {
    ...
    // 删除标签
    handleDel (row) {
      deleteProfessional(row.id, row).then((result) => {
