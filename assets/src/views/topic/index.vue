<template>
  <div>
    <h4 class="font-weight-bold py-2 mb-2">話題管理</h4>
    <list-filter
...
export default {
  ...
  methods: {
    ...
    // 保存标签
    handleCreateData () {
      createTopic({ ...this.userForm }).then((result) => {
    ...
    handleUpdateData () {
      ...
      updateTopic(id, { ...this.userForm }).then((result) => {
    ...
    // 删除标签
    handleDel (row) {
      deleteTopic(row.id, row).then((result) => {
