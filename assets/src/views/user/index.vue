<template>
  <div>
    <h4 class="font-weight-bold py-2 mb-2">{{$t('TITLE_USER_MANAGER')}}</h4>
    <list-filter
      :filters="listQuery.filters"
      @action="handleFilter"
      :filterOptions="filterOptions"
    />
    <b-btn variant="primary rounded-pill" class="d-block mb-3" @click="showUserCreateModal">
...
import { getUserList, createUser, updateUser, deleteUser, getRoles } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    handleUpdateData () {
      ...
      updateUser(id, this.userForm).then((result) => {
    ...
    // 删除用户
    handleDel (row) {
      deleteUser(row.id, row).then((result) => {
